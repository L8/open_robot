...
public class ThumbBall extends View implements OnTouchListener {
	 ...
	 public boolean onTouch(View v, MotionEvent e) {
         ...
         case MotionEvent.ACTION_MOVE:
         	...
         	if (GeometryHelper.inCircle(this.x, this.y, this.r, touchX, touchY) && 
         			touchX >= 0 &&
         			touchX <= TouchControl.FRAME_WIDTH &&
         			touchY >= 0 &&
         			touchY <= TouchControl.FRAME_HEIGHT) {
         			this.x = touchX;
         			this.y = touchY;
         			this.invalidate();
	...
	private int boxCenterX() {
		return TouchControl.FRAME_WIDTH / 2;
	}
	 
	private int boxCenterY() {
		return TouchControl.FRAME_HEIGHT / 2;
	}
